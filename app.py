...
# =====================================================
    ...
    st.write("Silakan gunakan *slider* di bawah ini untuk menguji sensitivitas nilai ekonomi kawasan berdasarkan perubahan asumsi data di lapangan atau dinamika harga pasar.")
    ...
    with col_s1:
        luas_sim = st.slider("Luas Hutan Efektif (ha)", 25000, 35000, int(luas_hutan), step=500)
        produksi_sim = st.slider("Produksi Kayu Tahunan (m³/tahun)", 20000, 45000, produksi, step=1000)
    with col_s2:
    ...
    # Kalkulasi Berdasarkan Input Simulasi
    ...
    c3.metric("Total Valuasi Kawasan",  f"Rp {sim_total/1e9:,.2f} Milyar", f"Skenario Optimal")

    st.markdown("""
        <div class="desc-box">
        <i>*Catatan: Pendapatan bersih kayu (Rente Ekonomi) dihitung dengan asumsi margin laba bersih operasional (Profit Margin) sebesar 35% dari total nilai bruto.</i>
        </div>
    """, unsafe_allow_html=True)
